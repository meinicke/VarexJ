    ...
    void checkVarInsn (LocalVariableInstruction insn){
      if (varChecks != null){
        ...
        for (int i = 0; i < varChecks.length; i++) {
          ...
          if (lvar != null) {
            long v = lvar.getSlotSize() == 1 ? frame.getLocalVariable(CachedFeatureExprFactory.True(), slotIdx).getValue() : frame.getLongLocalVariable(CachedFeatureExprFactory.True(), slotIdx);
